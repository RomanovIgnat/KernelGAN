...
from torchZSSR.torchZSSRconfigs import Config  # !!! other config
from torchZSSR.torchZSSRutils import *
from torchZSSR.torchZSSRnet import simpleNet
import numpy as np
import torch
...
class ZSSR:
    ...
    def __init__(self, input_img_path, scale_factor=2, kernels=None, is_real_img=False, noise_scale=1.):
        ...
        self.gt = None  # ground_truth if type(ground_truth) is not str else img.imread(ground_truth)
        # gt is ndarray
        # Preprocess the kernels. (see function to see what in includes).
        self.kernels = preprocess_kernels(kernels, self.conf)
